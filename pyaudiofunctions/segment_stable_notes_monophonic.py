...
def segment_stable_notes_monophonic(inputFile = '../../sounds/sax-phrase-short.wav', stdThsld=10, minNoteDur=0.1, 
    ...
    std_winStable = [ np.std(cent_f0[index-winStable: index]) for index in range(winStable, cent_f0.size+1)]
    ...
    initial = [ x  for x in contiguous_index[0] if x-1 not in contiguous_index[0] and x+1 in contiguous_index[0]]
    final = [ x  for x in contiguous_index[0] if x-1 in contiguous_index[0] and x+1 not in contiguous_index[0]]
